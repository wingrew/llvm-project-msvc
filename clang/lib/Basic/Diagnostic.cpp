...
DiagnosticsEngine::DiagState *
DiagnosticsEngine::DiagStateMap::File::lookup(uint64_t Offset) const {
  auto OnePastIt =
...
DiagnosticsEngine::DiagStateMap::getFile(SourceManager &SrcMgr,
  ...
  // initialize it.
  if (ID.isValid()) {
    std::pair<FileID, uint64_t> Decomp = SrcMgr.getDecomposedIncludedLoc(ID);
...
void DiagnosticsEngine::DiagStateMap::dump(SourceManager &SrcMgr,
  ...
  for (auto &F : Files) {
    ...
    auto PrintOuterHeading = [&] {
      ...
      if (F.second.Parent) {
        std::pair<FileID, uint64_t> Decomp =
